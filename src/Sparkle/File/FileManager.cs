...
public static class FileManager {
    ...
    public static void CreateFile(string directory, string name, bool overwrite = false) {
        ...
        if (overwrite || !System.IO.File.Exists(GetPath(directory, name))) {
            System.IO.File.Create(GetPath(directory, name)).Close();
        }
    }
    ...
    public static JObject ReadJsonAsObject(string path, string key = "") {
        return JObject.Parse(DecryptString(System.IO.File.ReadAllText(path), key));
    }
